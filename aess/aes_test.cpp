...
*/
	...
	AES_ECB_encrypt(in, key, buffer, sizeof(buffer));
	
	
	printf("encrypt data:\n");
	printOutMsg(buffer, sizeof(buffer));


	uint8_t outData[64*3];

	AES_ECB_decrypt(buffer, key, outData, sizeof(buffer));


	printf("decrypt data:\n");
	printOutMsg(outData, 64*3);
...
static void test_encrypt_cbc(void)
{
  ...
  printf("original data:\n");
  printOutMsg(in, 16*16);

  AES_CBC_encrypt_buffer(buffer, in, 16*16, key, iv);


  printf("encrypt data:\n");
  printOutMsg(buffer, 16*16);
...
/*  printf("CBC encrypt: ");
  ...
  else
  {
	...
	uint8_t outData[16*16];

	AES_CBC_decrypt_buffer(outData, buffer, 16*16, key, iv);

	printf("decrypt data:\n");
	printOutMsg(outData, 16*16);
...
void AES_CBC_encrypt_decrypt(void)
{
	...
	//store the encrypt data,the length is the same with in
	uint8_t buffer[16*16*2];

	printf("original data:\n");
	printOutMsg(in, sizeof(in));

	AES_CBC_encrypt_buffer(buffer, in, sizeof(in), key, iv);

	printf("encrypt data:\n");
	printOutMsg(buffer, sizeof(buffer));

	uint8_t outData[16*16*2];

	AES_CBC_decrypt_buffer(outData, buffer, sizeof(buffer), key, iv);

	printf("decrypt data:\n");
	printOutMsg(outData, sizeof(outData));
